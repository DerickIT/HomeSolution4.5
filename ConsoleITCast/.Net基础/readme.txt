...
字符编码：gb2312，utf—8（国际码表英1，汉3），Unicode
 ...
 大文件拷贝两种方式，1是将文件全部压倒内存中，而是通过流的方式，通过文件的偏移位移，读取一行，处理一行的流操作
 加解密文件流：在缓冲区中改变字符的偏移量
 由于filestream可能会丢失字节，读取一行按照字符，所以通常会使用streamreader
 正则表达式
 序列化对象-1可以直接使用，2在网络中可以直接发送对象信息
 序列化对象-创建binaryfomatter-创建流-调用方法serialize
